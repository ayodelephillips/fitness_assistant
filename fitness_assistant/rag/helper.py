...
def format_vector_db_context(context_points:list[ScoredPoint]):
    ...
    """
    context_mapping = LlmConfig().context_mapping
    context_texts = []
...
#     format_vector_db_context(con)


########### enforce schema validation on the vector db response. check for empty response for example