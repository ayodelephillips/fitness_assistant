...
def test_integration_personalization():
    ...
    print("✓ Retrieved profile from manager")
    ...
    print("✓ Updated profile in manager")
    ...
    # Test with beginner profile with leg restriction
    beginner_profile = UserProfile(
        fitness_level=FitnessLevel.BEGINNER, restrictions=["legs"]
    )
    filtered = filter_exercises_by_profile(sample_exercises, beginner_profile)
    print(
        f"✓ Filtered exercises for beginner with knee restriction: {len(filtered)} exercises"
    )
